...
int calculateExcellentStudentsPercentage(Student* arr, int n) {
...
double printStudentsWithExcellentPhysics(Student* arr, int n) {
    int five = 0; //кількість студентівшо маюиь 5
    for (int i = 0; i < n; i++) {
        if (arr[i].return_physics() == 5) {
            five++;
        }
...
int main()
{
    ...
    cout << "Кільксіть студентів-відмінників: " << calculateExcellentStudentsPercentage(arr, n) << endl;
    cout << "Процент студентів що отримали 5 з фізики:" << printStudentsWithExcellentPhysics(arr, n) << '%' << endl;
